...
echo "\$color-accent-0-0 =$color1" > $temp_file
echo "\$color-accent-0-1 =$color2" >> $temp_file
echo "\$color-accent-0-2 =$color3" >> $temp_file

# Append the rest of the hyprland file to the temporary file
cat ~/.config/hypr/hyprland.conf | tail -n +4 >> $temp_file

# Move the temporary file to the config file
mv $temp_file ~/.config/hypr/hyprland.conf